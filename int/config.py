...
class Config:
    ...
    @classmethod
    def is_placeholder(cls, key: str) -> bool:
        """Determines if the given key value is a placeholder/mock value."""
        val = key.strip()
        return val in {
            "", "mock_ocean_key", "mock_prospeo_key", "mock_eazyreach_key", "mock_brevo_key",
            "your_ocean_api_key_here", "your_prospeo_api_key_here", "your_eazyreach_api_key_here", "your_brevo_api_key_here"
        }
    ...
    def get_missing_credentials(cls) -> list[str]:
        ...
        missing = []
        if cls.is_placeholder(cls.OCEAN_API_KEY):
            missing.append("OCEAN_API_KEY")
        if cls.is_placeholder(cls.PROSPEO_API_KEY):
            missing.append("PROSPEO_API_KEY")
        if cls.is_placeholder(cls.EAZYREACH_API_KEY):
            missing.append("EAZYREACH_API_KEY")
        if cls.is_placeholder(cls.BREVO_API_KEY):
